...
def main(google_drive_url):
    try:
        # Шаг 1: Извлечение и исправление текста с изображения
        logger.info("Запуск обработки текста с изображения...")
        image = download_image_from_gdrive(google_drive_url)  # Загружаем изображение
        process_image_text(google_drive_url)  # Извлекаем и исправляем текст
        
        # Шаг 2: Преобразование изображения в байты для дальнейшей обработки
